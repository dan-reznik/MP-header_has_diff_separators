...
In the directory "data_many" I placed 3 slightly modified copies of the original file with the problematic header:
...
Let's build a table showing in each line the name of the file, the first line, and the 2nd line:
...
As before let us fix the separators in the header for the first file in the series
...
Now let's read all files with the fixed header and concatenate them in a single data frame:
